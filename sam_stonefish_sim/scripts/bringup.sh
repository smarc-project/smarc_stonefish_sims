...
do
	...
	tmux new-window -t $ROBOT_SESSION:5 -n 'dr'
    tmux new-window -t $ROBOT_SESSION:6 -n 'static_ctrl'
    tmux new-window -t $ROBOT_SESSION:7 -n 'dynamic_ctrl'
	...
	tmux send-keys "mon launch tf_convenience_topics odom_listener.launch robot_name:=$ROBOT_NAME --name=${ROBOT_NAME}_$(tmux display-message -p 'p#I_#W') --no-start" C-m
